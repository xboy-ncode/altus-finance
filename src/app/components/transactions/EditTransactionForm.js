...
export default function EditTransactionForm({ transaction, onUpdateTransaction, onCancel }) {
    ...
    const [formData, setFormData] = useState({
        ...
        toAmount: '', // Transfers not supported for editing yet
        date: transaction?.date ? new Date(transaction.date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
        type: transaction?.amount < 0 ? 'Gasto' : 'Ingreso',
        categoryId: transaction?.categoryId || '',
        merchant: transaction?.merchant || '',
        accountId: transaction?.accountId || '', // We need accountId in transaction object
        toAccountId: '',
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            ...
            // Set defaults if not provided in transaction (edge case)
            if (!formData.accountId && accs.length > 0) setFormData(prev => ({ ...prev, accountId: accs[0].id }));
            if (!formData.categoryId && cats.length > 0) {
                const typeCat = cats.find(c => c.type === (formData.type === 'Ingreso' ? 'income' : 'expense'));
                if (typeCat) setFormData(prev => ({ ...prev, categoryId: typeCat.id }));
            }
        };
        loadInitialData();
    }, [formData.accountId, formData.categoryId, formData.type]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (isTransfer) {
            alert('La edición de transferencias no está soportada aún.');
            return;
        ...
        try {
            ...
            if (result.success) {
                ...
                alert(result.error || 'Error al actualizar la transacción');
            }
        } catch (error) {
            console.error(error);
    ...
    return (
        <Dialog open={true} onOpenChange={(open) => { if (!open && !loading) onCancel(); }}>
            <DialogContent className="sm:max-w-[500px] overflow-y-auto max-h-[90vh]">
                <DialogHeader>
                    <DialogTitle>{t('transactions.editTitle', 'Editar Transacción')}</DialogTitle>
                </DialogHeader>
